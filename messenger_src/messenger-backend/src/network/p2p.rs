...
impl NodeHandle {
    pub async fn start(
        ...
        identity_source: std::sync::Arc<dyn PeerIdentitySource>,
        // РАНЬШЕ генерировался тут же через
        // `x25519_dalek::StaticSecret::random_from_rng` на каждом
        // старте — значит опубликованный PreKeyBundle всегда бы
        // ссылался на identity-ключ, который умирает при следующем
        // рестарте (собеседник больше не смог бы завершить X3DH с этим
        // бандлом). Теперь вызывающий код (main.rs / src-tauri/lib.rs)
        // отвечает за load-or-generate через storage::db::save_x3dh_keys
        // /load_x3dh_keys, тем же паттерном что и identity выше.
        x3dh_identity: x25519_dalek::StaticSecret,
        // Тем же паттерном что и x3dh_identity выше: раньше SessionManager
        // вообще не хранил ни signed prekey, ни one-time prekeys — из-за
        // этого `SessionManager::accept_incoming_session` физически не
        // мог посчитать тот же DH, что уже посчитал инициатор, когда
        // приходило первое сообщение от нового собеседника (см.
        // session/manager.rs). Вызывающий код (main.rs / src-tauri/lib.rs)
        // передаёт сюда ровно ту же пару ключей, которую публикует в
        // своём PreKeyBundleRecord.
        our_signed_prekey: x25519_dalek::StaticSecret,
        our_one_time_prekeys: Vec<x25519_dalek::StaticSecret>,
        // РАНЬШЕ генерировался тут же случайно на каждом старте (см. ниже,
        // где раньше стояло `StaticSecret::random_from_rng`) — значит если
        // этот узел прописан в чьём-то cfg.static_relays с нашим
        // onion_public_key_hex, наш onion-слой для них ломался бы при
        // каждом рестарте (см. storage/db.rs::save_onion_key/load_onion_key
        // и комментарий у CREATE TABLE local_onion_key). Вызывающий код
        // (main.rs / src-tauri/lib.rs) теперь отвечает за load-or-generate,
        // тем же паттерном что и x3dh_identity/our_signed_prekey выше.
        our_onion_secret: x25519_dalek::StaticSecret,
        ...
        for relay in &cfg.static_relays {    
            if let Ok(addr) = relay.address.parse::<Multiaddr>() {    
                if let Err(e) = swarm.dial(addr.clone()) {    
                    tracing::warn!("Не удалось подключиться к relay {} ({addr}): {e}", relay.relay_id);    
                }    
            } else {    
                tracing::warn!("Невалидный multiaddr у relay {}: {}", relay.relay_id, relay.address);    
            }    
        }    
        ...
        for relay in &cfg.static_relays {    
            if let Err(e) = database.upsert_relay_reputation(&relay.relay_id, &relay.address, 0.0) {    
                tracing::warn!("Не удалось засеять relay {} в базу: {e}", relay.relay_id);    
            }    
        }    

        let onion_key_source: std::sync::Arc<dyn crate::network::relay::scoring::OnionKeySource> =    
            std::sync::Arc::new(crate::network::relay::StaticOnionKeySource::from_config(    
                &cfg.static_relays,    
            ));    
